...
# The crt-build script is used to determine build metadata and create vault-mcp-server builds.
...
function build() {
  ...
  : "${BIN_PATH:="dist/vault-mcp-server"}" #if not run by actions-go-build (enos local) then set this explicitly
  : "${GO_TAGS:=""}"
  : "${KEEP_SYMBOLS:=""}"

  # Build our ldflags
  msg="--> Building vault-mcp-server revision $revision, built $build_date"
  ...
  ldflags="${ldflags} -X vault-mcp-server/version.GitCommit=$revision -X vault-mcp-server/version.BuildDate=$build_date"

  if [[ ${BASE_VERSION+x} ]]; then
    msg="${msg}, base version ${BASE_VERSION}"
    ldflags="${ldflags} -X vault-mcp-server/version.Version=$BASE_VERSION"
  fi

  if [[ ${PRERELEASE_VERSION+x} ]]; then
    msg="${msg}, prerelease ${PRERELEASE_VERSION}"
    ldflags="${ldflags} -X vault-mcp-server/version.VersionPrerelease=$PRERELEASE_VERSION"
  fi

  if [[ ${METADATA_VERSION+x} ]]; then
    msg="${msg}, metadata ${METADATA_VERSION}"
    ldflags="${ldflags} -X vault-mcp-server/version.VersionMetadata=$METADATA_VERSION"
  fi
