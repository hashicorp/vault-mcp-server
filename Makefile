...
# Build flags
LDFLAGS=-ldflags="-s -w -X vault-mcp-server/version.GitCommit=$(shell git rev-parse HEAD) -X vault-mcp-server/version.BuildDate=$(shell git show --no-show-signature -s --format=%cd --date=format:"%Y-%m-%dT%H:%M:%SZ" HEAD)"
